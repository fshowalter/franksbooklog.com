...
describe("markdownToDescription", () => {
  it("returns a short body whole", ({ expect }) => {
    expect(markdownToDescription("Short body.")).toBe("Short body.");
  });

  it("collapses paragraph breaks to spaces", ({ expect }) => {
    expect(markdownToDescription("One.\n\nTwo.")).toBe("One.  Two.");
  });

  it("truncates a long body to at most 160 characters", ({ expect }) => {
    expect(markdownToDescription(LONG_BODY).length).toBeLessThanOrEqual(160);
  });

  it("truncates on a word boundary rather than mid-word", ({ expect }) => {
    expect(markdownToDescription(LONG_BODY)).toBe(
  ...
  it("strips markdown syntax before measuring", ({ expect }) => {
    expect(markdownToDescription("A *very* [linked](/x) body.")).toBe(
      "A very linked body.",
    );
  });
});
